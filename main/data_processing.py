...
def AA_search_8ion(peaks, prefix_masses, pep_mass, direction="fw"):
    ...
    for i, aa in enumerate(AAs):
        ...
        cand_int[
            r, i, c // len(peak_masses), ((window_size // 2) + np.floor(AA_diffs * (window_size))).astype(np.int32)] = \
        peak_ints[c % len(peak_masses)]
...
def process_spectrum_Graph(spectrum):
    ...
    cand_int = AA_search_8ion(peaks=peaks_forward,
                              ...
                              pep_mass=peptide_mass + model_utils.mass_N_terminus + model_utils.mass_C_terminus + (
        ...
        if (data_format == "msp"):
            ...
            if len(mods) > 0:
                ...
                for mod in mods:
                    ...
                    if modtype == "Acetyl":
                        raw_sequence = raw_sequence[:int(pos) + added_len] + "(+42.0106)" + raw_sequence[
                                                                                            int(pos) + added_len:]
                        added_len += 10
                    elif modtype == "Oxidation":
                        raw_sequence = raw_sequence[:int(pos) + added_len] + "(+15.99)" + raw_sequence[
                                                                                          int(pos) + added_len:]
                        added_len += 8

                    elif modtype == "CAM":
                        raw_sequence = raw_sequence[:int(pos) + added_len] + "(+57.02)" + raw_sequence[
                                                                                          int(pos) + added_len:]
                        added_len += 8
                    elif modtype == "Amide":
                        raw_sequence = raw_sequence[:int(pos) + added_len] + "(+.98)" + raw_sequence[
                                                                                        int(pos) + added_len:]
                        added_len += 6
                    else:
                        raw_sequence = raw_sequence[:int(pos) + added_len] + "(+0)" + raw_sequence[
                                                                                      int(pos) + added_len:]
                        added_len += 4
            # MW
            line = file_handle.readline()
